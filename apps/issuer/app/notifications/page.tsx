...
export default function NotificationsPage() {
    ...
    useEffect(() => {
        if (notifications.length > 0) {
            try { localStorage.setItem('notifications', JSON.stringify(notifications)); } 
            catch {}
        }
    }, [notifications]);
            ...
            <div className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 sticky top-0 z-10">
                <div className="max-w-4xl mx-auto px-4 py-4">
                    <div className="flex items-center justify-between">
                        ...
                        {unreadCount > 0 && (
                            <button onClick={markAllAsRead} className="text-sm text-purple-600 hover:text-purple-700 dark:text-purple-400 font-medium px-4 py-2 min-h-[44px] rounded-lg hover:bg-purple-100 dark:hover:bg-purple-900/30 transition-colors">
            ...
            <div className="max-w-4xl mx-auto px-4 py-6">
                {notifications.length === 0 ? (
                    <div className="bg-white dark:bg-gray-800 rounded-2xl p-12 text-center shadow-sm">
                        ...
                        <p className="text-sm text-gray-500 dark:text-gray-500 mt-2">Vous êtes à jour !</p>
                    </div>
                ) : (
                    <div className="space-y-3">
                        {notifications.map((notification) => (
                            <div key={notification.id} onClick={() => markAsRead(notification.id)} className={`bg-white dark:bg-gray-800 rounded-2xl p-5 cursor-pointer transition-all shadow-sm hover:shadow-md ${notification.read ? '' : 'ring-2 ring-purple-200 dark:ring-purple-800'}`}>
